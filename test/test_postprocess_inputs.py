...
from NanoCore import Atom, AtomsSystem, s2
...
class InputTests(unittest.TestCase):
    ...
    def test_band_eig_inputs_and_common_schema(self):
        ...
        for function, suffix, alias in [(s2.get_band, 'bands', 'bands_path'),
                                        (s2.get_eig, 'EIG', 'eig_path')]:
    ...
    def test_dos_paths_and_legacy_return(self):
        def run(command):
            ...
            return 0
        with mock.patch.object(s2.os, 'system', side_effect=run):
            expected = 'sample.EIG'
            self.assertEqual(s2.get_dos(-1, 1, simobj=self.sim), ([0], [3], [1], [2]))
            expected = 'folder with spaces/input.EIG'
            s2.get_dos(-1, 1, simobj=self.sim, file_path=expected)
    ...
    def test_pdos_quantum_selection_and_saved_output(self):
        ...
        with mock.patch.object(s2.subprocess, 'run', side_effect=run):
            for quantum, expected in [({}, ['O', '0']),
                                      ({'n': 2}, ['O', '2', '-1']),
                                      ({'n': 2, 'l': 1}, ['O', '2', '1', '9']),
                                      ({'n': 2, 'l': 1, 'm': 0}, ['O', '2', '1', '0'])]:
                result = s2.get_pdos(file_path='sample.PDOS', species=['O'],
                                     output_path='O_selected', **quantum)
                self.assertEqual(result, ([-1, 0], [2, 3], []))
                self.assertTrue(Path('O_selected').is_file())
        with mock.patch.object(s2.subprocess, 'run') as run:
            with self.assertRaises(FileNotFoundError):
                s2.get_pdos(file_path='sample.PDOS', atom_index=[1])
            with self.assertRaises(ValueError):
                s2.get_pdos(file_path='sample.PDOS', atom_index=[1], output_path='sample.PDOS')
    ...
    def test_pdos_inputs_and_pldos_structure_override(self):
        ...
        with mock.patch.object(s2.subprocess, 'run', side_effect=run):
            ...
            self.assertEqual(s2.get_pdos(simobj=self.sim, atom_index=[1]), ([0, 1], [2, 4], [3, 5]))
            z, dos, energy = s2.get_pldos(simobj=self.sim)
            ...
            for sim in (None, self.sim):
                z, dos, energy = s2.get_pldos(simobj=sim, file_path=expected, structure_path='input.xyz')
                self.assertEqual(z, [2])
                np.testing.assert_array_equal(dos, [[2], [4]])
        with self.assertRaises(ValueError):
            s2.get_pldos(file_path='sample.PDOS')
